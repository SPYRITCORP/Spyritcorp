...
function showCartNotification(productName, productImage) {
    ...
    notification.querySelector('.notification-text').textContent = 
        `¡${productName} fue añadido al carrito!`;
    notification.querySelector('.notification-img').src = productImage;
...
// Asumiendo que los botones tienen la clase .button-add-to-cart
document.querySelectorAll('.button-add-to-cart').forEach(btn => {
    btn.addEventListener('click', function() {
        const product = this.closest('.product');
        const name = product.getAttribute('data-name');
        const image = product.getAttribute('data-image');
        showCartNotification(name, image);

        // Aquí va el resto de tu lógica para añadir al carrito...
    });
});
